...
//! Shared display helpers for StaticWeaver examples.
//!
//! Provides animated spinner + checkmark output inspired by
//! charmbracelet/bubbletea's package-manager example. Each example binary
//! re-includes this file via `#[path = "support.rs"] mod support;` so the
//! helpers stay colocated without a published sub-crate.
